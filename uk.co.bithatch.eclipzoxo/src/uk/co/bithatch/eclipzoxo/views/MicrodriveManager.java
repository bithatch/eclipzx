...
import uk.co.bithatch.zoxo.interface1.Interface1;
import uk.co.bithatch.zoxo.interface1.Interface1DriveListener;
...
public class MicrodriveManager extends Composite implements Interface1DriveListener {
	...
	private MicrodriveManager(Composite parent, int style, Interface1 iface1, Interface1Settings settings, Path fileToMount) {
		super(parent, style);
		...
		colFile.setLabelProvider(new ColumnLabelProvider() {
			...
			public String getText(Object element) {
				...
				if (cart.isInserted()) {
					return cart.getFile() == null ? "Unformatted Cartridge" : cart.getFile().getFileName().toString();
				} else {
					return "Empty Drive";
				}
			}
		...
		// Seed rows
		viewer.setInput(seed(Byte.toUnsignedInt(settings.getMicrodriveUnits())));
		...
		if(fileToMount != null) {
			parent.getDisplay().asyncExec(() -> {
				...
				for(int i = 0 ; i < carts.size(); i++) {
					...
					if(cart.isInserted()) {
						if(fileToMount.toAbsolutePath().equals(cart.getFile().toAbsolutePath())) {
							return;
						}
					}
					else  {
	...
	private void updateEntry(Cartridge entry) {
		...
		entry.setFile(drvPath == null || drvPath.equals("") ? null : Paths.get(drvPath));
	...
	private void onOpenFileSystem() {
		...
		if (res != null) {
			mountFileToCartridge(sel, Paths.get(res));
		}
	}

	private void mountFileToCartridge(Cartridge sel, Path resFile) {
		lastPath = resFile.getParent().toAbsolutePath().toString();
		if (iface1.insertFile(sel.getDrive() - 1, resFile.toFile())) {
			driveChanged(sel);
		} else {
			MessageDialog.openError(getShell(), "Cartridge Error", "Failed to open tape cartridge.");
		}
	...
	private void onSaveAs(Cartridge row) {
		...
		fileDialog.setFilterPath(row.getFile() == null ? lastPath : row.getFile().getParent().toAbsolutePath().toString());
		fileDialog.setText("Select File");
		fileDialog.setFilterExtensions(new String[] { "*.mdr;*.mdv", "*.*" });
		fileDialog.setFilterNames(new String[] { "Microdrive Cartridge File (*.mdr,*.mdv)", "All Files (*.*)" });
		fileDialog.setFileName(row.getFile() == null ? "Untitled.mdr" : row.getFile().getFileName().toString());
		var res = fileDialog.open();
		if (res != null) {
			var resFile = Paths.get(res);
			lastPath = resFile.toAbsolutePath().toString();
			if (!iface1.save(row.getDrive() - 1, resFile.toFile())) {
	...
	public static void openDialog(Shell parent, Interface1 iface1, Interface1Settings settings) {
		openDialog(parent, iface1, settings, null);
	}
	
	public static void openDialog(Shell parent, Interface1 iface1, Interface1Settings settings, Path fileToMount) {
