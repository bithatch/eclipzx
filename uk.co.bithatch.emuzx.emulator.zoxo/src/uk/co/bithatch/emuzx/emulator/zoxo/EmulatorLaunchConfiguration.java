...
public class EmulatorLaunchConfiguration extends AbstractPreparedLaunchConfigurationDelegate<IInternallyLaunchable> {
	...
	protected void preparedLaunch(ILaunchConfiguration configuration, ILaunch launch, IProgressMonitor monitor,
		...
		var ofmt = launchable.getLaunchFormat(configuration, file, f -> Activator.ZOXO_RUNNABLE_FORMATS.contains(f.extension()));
		...
		if(mode.equals("debug")) {
			var debugInfo = LanguageSystem.languageSystem(prepCtx.programFile()).createSourceAddressMap(binaryFile);
			PlatformUI.getWorkbench().getDisplay().execute(() -> {
				...
				try {
					...
					launch.addDebugTarget(new EmulatorDebugTarget(launch, eview, debugInfo));
	...
	protected Predicate<IOutputFormat> getSupportedFormatsFilter() {
		...
		return f -> sf.test(f) && Activator.ZOXO_RUNNABLE_FORMATS.contains(f.extension());
	}
