...
import static uk.co.bithatch.emuzx.emulator.zoxo.EmulatorLaunchConfigurationAttributes.OUTPUT_FORMAT;
	...
	protected void updateOutputFormats() {
		...
		if(lang == null)
			...
			var formats = arch.supportedFormats().stream()
				.filter(f -> Activator.ZOXO_RUNNABLE_FORMATS.contains(f.extension())).toList();
