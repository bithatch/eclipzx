...
import uk.co.bithatch.emuzx.debug.DezogZ80Thread;
 ...
 */
public final class EmulatorZ80Thread extends DelegatingDebugElement implements IThread {

	private static final ILog LOG = ILog.of(EmulatorZ80Thread.class);

	private final EmulatorInstance rsp;
	 ...
	 */
	private final EmulatorZ80StackFrame currentFrame;
	private volatile boolean frameDirty = true;

	EmulatorZ80Thread(IDebugTarget delegate, EmulatorInstance rsp, ISourceAdressMap debugInfo) {
		super(delegate);
		this.rsp = rsp;
		this.debugInfo = debugInfo;
		this.currentFrame = new EmulatorZ80StackFrame(this, rsp, debugInfo);
	}

	private EmulatorDebugTarget	 target() {
		return (EmulatorDebugTarget) super.delegate();
	}
